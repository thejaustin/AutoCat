...
fun DrawerSearchPreference(
    ...
    MainSwitchPreference(
        ...
        PreferenceGroup(heading = stringResource(id = R.string.show_search_result_types)) {
            ...
            if (searchAlgorithm != AutoCatSearchAlgorithm.ASI_SEARCH) {
                val navController = LocalNavController.current
                val canDisable = searchAlgorithm != AutoCatSearchAlgorithm.APP_SEARCH
            ...
            when (searchAlgorithm) {
                AutoCatSearchAlgorithm.LOCAL_SEARCH -> {
                    LocalSearchSettings(
                        prefs = prefs,
                        prefs2 = prefs2,
                        context = context,
                ...
                AutoCatSearchAlgorithm.ASI_SEARCH -> {
...
private fun SearchProvider(
    ...
    val searchAlgorithmEntries = remember {
        sequenceOf(
            ListPreferenceEntry(AutoCatSearchAlgorithm.APP_SEARCH) { stringResource(R.string.search_algorithm_app_search) },
            ListPreferenceEntry(AutoCatSearchAlgorithm.LOCAL_SEARCH) { stringResource(R.string.search_algorithm_global_search_on_device) },
            ListPreferenceEntry(AutoCatSearchAlgorithm.ASI_SEARCH) { stringResource(R.string.search_algorithm_global_search_via_asi) },
            ListPreferenceEntry(AutoCatSearchAlgorithm.SEMANTIC_SEARCH) { "Semantic Search (AI-Powered)" },
        ).filter {
            when (it.value) {
                AutoCatSearchAlgorithm.ASI_SEARCH -> AutoCatSearchAlgorithm.isASISearchEnabled(
                    context,
                )
