...
class OpenAIProvider(
    ...
    override suspend fun categorizeApp(
        ...
        try {
            ...
            LLMLogger.logInfo(
                provider = name,
                operation = "CATEGORIZE_APP",
                message = "Successfully categorized $appName as ${result.tabName}",
                details = mapOf(
                    "package" to appPackage,
    ...
    override suspend fun suggestTabs(
        ...
        try {
            ...
            throw LLMException("OpenAI tab suggestion failed: ${e.message}", e)
        }
    }
    ...
    override suspend fun suggestFolders(
        ...
        try {
            ...
            throw LLMException("OpenAI folder suggestion failed: ${e.message}", e)
        }
    }
  ...
  "results": {
    ...
    private fun parseResponse(
        ...
        try {
            ...
            val tabName = result.getString("tab") // LLM still returns 'category'
    ...
    private fun parseFolderSuggestionResponse(responseJson: String): List<SuggestedFolder> {
        try {
            ...
            throw LLMException("Failed to parse OpenAI folder suggestion response: ${e.message}", e)
        }
    }
    ...
    private fun parseBatchResponse(
        ...
        try {
            ...
            resultsObj.keys().forEach { packageName ->
                ...
                val tabName = appResult.getString("tab") // LLM still returns 'category'
